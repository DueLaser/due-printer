...
app.service('machineService', function($rootScope, $timeout, warningService) {
  ...
  var connect = function(port, baud) {
    ...
    console.log("Conectando em '" + port + "' com options: " + JSON.stringify(options));
    chrome.serial.connect(port, options, function(info) {
      if (!info) {
        warningService.warn("connection", "Não foi possivel conectar a '" + port + "'");
        api.isBusy = false;
  ...
  var enqueueCommands = function(cmds) {
    ...
    var homming = function() {
      api.commandQueue.push("$H\n");
      api.homado=true; 
      $rootScope.$apply();
  }
  ...
  var logCommand = function(cmd, isSend) {
    ...
    if (!isSend) {
      ...
      if ((incomingCommandLookbackChar + cmd).indexOf("Grbl") != -1) {
      api.homado=false;
      $rootScope.$apply();
      //sendCommandToSerialConnection("$H\n", true);
      }
      else if ((incomingCommandLookbackChar + cmd).indexOf("Reset") != -1) {
      api.commandQueue = [];
      sendCommandToSerialConnection("M05\n", true);
      sendCommandToSerialConnection("R\n", true);
      warningService.warn("cmd", "Limites de segurança excedidos, reiniciando...");
      }
  ...
  var sendCommandToSerialConnection = function(cmd) {
    ...
    if (!connectionId) {
      warningService.warn("cmd", "Não existe conexão.");
      return;
    }
    ...
    chrome.serial.send(connectionId, str2ab(cmd), function(info) {
      if (info.error) {
        warningService.warn("cmd", "Falhou ao enviar o comando: " + info.error);
      }
    });
  ...
  chrome.serial.onReceiveError.addListener(function(info) {
    warningService.warn("connection", "Erro na comunicacao serial: " + info.error);
    $rootScope.$apply();
  });
  ...
  var api = {
    ...
    sendCommandToSerialConnection: sendCommandToSerialConnection,
